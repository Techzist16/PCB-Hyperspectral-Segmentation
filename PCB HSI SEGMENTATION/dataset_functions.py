...
def read_hsi_cube(datapath, Cubepath):
    ...
    # Open the HSI data using ENVI and access the entire cube using 
    # bipfile
...
def read_dataset(dataset_path):
    ...
    for i in tqdm(range(1,54)):
        ...
        PCB_Masks.append(cv2.cvtColor(cv2.imread(str(maskspath) ),\
                                      cv2.COLOR_BGR2GRAY))
...
def Generate_Validation_data(training_list, HSI_cubes, seg_masks):
    """
    Generates training data for the given HS cubes and their
    corresponding masks without augmentation. The function for reading PCB-Vision HS
    training cubes and generating the training set.
    
    Parameters:
        training_list (list): A list of indices corresponding to PCB-Vision 
                              HS cubes and masks to be included in the training set.        
        HSI_cubes (list): A list of HS data cubes
        seg_masks (list): A list of the ground truth masks
        
    Returns:
        cubes (list): A list of the HS cubes 
        masks (list): A list of the masks
        
    """
    cubes = []
    masks = []
    for i in training_list:
        cubes.append(HSI_cubes[i-1])
        masks.append(seg_masks[i-1])
...
def data_augmentation(hsi_cube, mask, alpha_min=0.9, alpha_max=1.1, b_prime=60):
    ...
    transformations, including PCA-based augmentation.

    Args:
        hsi_cube (ndarray): A hyperspectral cube of shape (rows, columns, bands)
        mask (ndarray): A 2D mask of shape (rows, columns).
        alpha_min (float): Minimum scaling factor for PCA-based augmentation.
        alpha_max (float): Maximum scaling factor for PCA-based augmentation.
        b_prime (int): Number of principal components to keep for PCA-based augmentation.

    Returns:
        A tuple of two lists:
        - The first list contains augmented hyperspectral cubes.
        - The second list contains augmented masks.
    """
    ...
    # Convert hyperspectral cube to 2D array for PCA
    hsi_reshaped = hsi_cube.reshape(-1, hsi_cube.shape[-1])  # Flatten spatial dimensions, keep bands intact

    # Perform PCA-based data augmentation
    X_centered = hsi_reshaped - np.mean(hsi_reshaped, axis=0)
    pca = PCA(n_components=b_prime)
    pca.fit(X_centered)
    first_pc = pca.components_[0]

    # Augmentations
    for _ in range(2):  # Random rotation clockwise
        angle = 15  # Fixed angle for simplicity
    ...
    for _ in range(2):  # Random rotation counterclockwise
        angle = 15  # Fixed angle for simplicity
        M = cv2.getRotationMatrix2D((cols / 2, rows / 2), -angle, 1)
        rotated_hsi = cv2.warpAffine(hsi_cube, M, (cols, rows))
        rotated_mask = cv2.warpAffine(mask, M, (cols, rows), flags=cv2.INTER_NEAREST)
    ...
    if np.random.rand() < 0.5:  # Random vertical translation
    ...
    if np.random.rand() < 0.5:  # Random horizontal translation
    ...
    flipped_hsi = np.flip(hsi_cube, axis=0)  # Flip on the vertical axis
    flipped_mask = np.flip(mask, axis=0)
    augmented_cubes.append(flipped_hsi)
    augmented_masks.append(flipped_mask)

    flipped_hsi = np.flip(hsi_cube, axis=1)  # Flip on the horizontal axis
    flipped_mask = np.flip(mask, axis=1)
    augmented_cubes.append(flipped_hsi)
    augmented_masks.append(flipped_mask)

    # Perform PCA-based augmentation
    augmented_samples = []
    for i in range(hsi_reshaped.shape[0]):
        alpha = np.random.uniform(alpha_min, alpha_max)
        augmented_first_pc = first_pc * alpha
        augmented_sample = X_centered[i] + np.dot(augmented_first_pc, np.dot(X_centered[i], first_pc))
        augmented_samples.append(augmented_sample)
    
    augmented_samples = np.array(augmented_samples)
    augmented_samples = augmented_samples.reshape(hsi_cube.shape)  # Reshape to original dimensions

    # Combine original and PCA-augmented hyperspectral cubes
...
def evaluate_segmentation(ground_truth_masks, predicted_masks, num_classes):
    ...
    pixel_accuracy_per_class = true_positive_sum / \
                              (true_positive_sum + false_negative_sum)
    ...
    expected_accuracy = np.sum(true_positive_sum) * np.sum(confusion_matrix_sum, axis=1)\
                         / total_pixels**2
